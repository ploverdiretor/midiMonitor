...
window.addEventListener("load", async ()=>{
  ...
  let voiceNote = ['OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF','OFF']
  for(let i=0;i<16;i++){
    const index = 'voice'+i
  ...
  /*------------------------------------------- Note On Process-----------------------------------------------*/
  function noteOn(data,velo){
    for(let i=0;i<16;i++){
      if(voiceEnv[i]==='OFF'){
        voiceEnv[i]='ON'
        document.getElementById('env'+i+'val').innerHTML = velo
        document.getElementById('note'+i+'val').innerHTML = noteNum[data]
        document.getElementById('voice'+i).innerHTML = voice[i] = data
        break;
      }
    }
  }
